...
5. **"Recovered waste" needs a definition — and it is never summed across dimensions.** Recovered waste is computed PER DIMENSION (for each dimension, sum `max(0, waste_then − waste_now)` over the segments flagged in the baseline run for that dimension) and the suggestion is the **largest single dimension's** recovery, never the sum — the same rupee appears in the placement, age-group and time-slot breakdowns at once, so adding them would bill the client roughly three times for one saving. This mirrors how headline waste is defined (§1 #1).
   A recommended cut is not recovered money. The baseline is the last approved run before the billing period; the comparison run is the last approved run inside it. The system calculates a **suggested** amount and the admin confirms or edits it before it reaches an invoice.
