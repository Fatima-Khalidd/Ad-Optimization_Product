...
Stage 3 note (from Stage 1 Task 2 review, binding): the `MAX_UPLOAD_MB` hard error from PLAN §3 is enforced in `services/upload.create_upload` on the raw bytes (413) BEFORE `load_csv` runs; `load_csv` itself never checks file size. Loader normalisation: `campaign_id` is trimmed only (identifier); `age_group` is trimmed + lowercased and keeps hyphens ("25-34"); only placement/device/gender/time_slot get the space/hyphen→underscore collapse and alias mapping.

Stage 1 Task 3 ruling (binding, 2026-09-16): `account_avg` benchmark = Σspend/Σconversions over significant segments WITH conversions > 0. Hand numbers: A(84000,40) B(40000,50) D(6000,0 conv, 200 clicks) → benchmark 1377.78, A flagged high_cpa waste 28888.89, D flagged zero_conversions waste 6000, total 34888.89.

Stage 1 close-out (binding for Stage 3+): `ReportOut.total_spend` is the ACCOUNT total = `app.pipeline.analyzer.account_total_spend(df) -> float` (Σ spend over every loaded row), never a per-dimension total (dimensions may be partially populated, PLAN §1 #3); `recovery_pct = headline_waste / account_total_spend * 100`. `segment_metrics` needs NO migration: `ctr/cvr/roas` are derived from stored counts in `SegmentOut.from_row`, and `flag_reason` is derived as `"zero_conversions" if is_flagged and conversions == 0 else ("high_cpa" if is_flagged else None)`. Import the pipeline dataclass as `from app.pipeline.optimizer import Recommendation as PipelineRecommendation` to avoid colliding with the ORM `Recommendation`. `PipelineConfig.from_overrides` type-checks and range-checks values (ValueError), so services may pass admin JSON straight to it.

Stage 2 close-out (binding): `make_admin(db, email="admin@example.com", *, is_active=True)` — the keyword-only `is_active` is part of the contract. `api_env` in Stage 2 sets the bound engine + test settings ONLY; **Stage 3 must extend it with the tmp storage root** (the fixture contract's description assumes the finished version). The `api` fixture builds `TestClient(create_app())` without a context manager because the app has no lifespan today — if Stage 3+ adds one (e.g. for storage), that fixture must switch to `with TestClient(...) as c`. Password hashing is settings-driven: `env == "test"` uses a deliberately weak Argon2 profile; production strength is unchanged and asserted by a test.

Stage 8 backlog from the Stage 2 review (do not lose): (a) `get_remote_address` reads `request.client.host` only, so behind the Vercel→backend proxy every login shares ONE 5/minute bucket — run uvicorn with `--proxy-headers --forwarded-allow-ips=<proxy>` or key the limiter on a trusted forwarded header; (b) no `Retry-After` header on 429; (c) no server-side refresh-token revocation — logout only clears cookies; (d) signup's spec-mandated 409 confirms whether an email is registered (accepted enumeration surface; login is uniform 401 and always pays a dummy hash verify).

## Stage 3 close-out (binding for Stages 4–8)

1. **`ReportOut.total_spend` is the ACCOUNT total**, persisted on the run as `analysis_runs.account_total_spend` (added by the Stage 3 close-out migration) from `analyzer.account_total_spend(df)`. It is NOT `max(per-dimension total_spend)` — on partially-populated exports (PLAN §1 #3) that understates spend (measured: 1500 vs 3500 on `partial_dimensions.csv`) and inflates `recovery_pct` by the same factor. `recovery_pct = headline_waste / account_total_spend * 100`, a `Decimal` quantized to 2 places, so it arrives in JSON as a STRING (e.g. `"16.28"`) — the frontend formats it and must not assume a number. Per-dimension `total_spend` remains the spend on rows that have that dimension.
2. **Money in JSON is always a string** (`-?\d+\.\d{2}`). `SegmentOut.ctr`/`cvr` are `0.0` (not null) when the denominator is zero; only `roas` is null when spend is 0 — the dashboard must show a dash for `impressions == 0` rather than "0.0%".
3. **Analyze is idempotent for an in-flight run**: `POST /api/analyze/{upload_id}` returns the SAME run (202) while an earlier run for that client+upload is `queued`/`running` and younger than the reuse window; a `done`/`failed` run allows a fresh one. Stage 4's Analyze button may therefore be clicked twice safely.
4. **`GET /api/reports/latest` answers 404** (not 204/null) when the client has no approved report — Stage 4's empty state keys off the 404, as do all other client-facing "not found" cases.
5. **`GET /api/uploads/template.csv` is public** (no session cookie) — it contains no tenant data. Every other route under `/api/uploads`, `/api/analyze`, `/api/runs`, `/api/reports` requires a client session; an admin gets 403 on them.
